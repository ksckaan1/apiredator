<script lang="ts">
  ...
  interface Props {
    ...
    bookmarkTags: string[];
    editBookmark: (bookmarkTitle: string, bookmarkTags: string[]) => void;
  }
  ...
  let inBookmarkTitle = $state(bookmarkTitle);
  let inBookmarkTags: string[] = $state(bookmarkTags);
  ...
  const onAddButtonClicked = () => {
    ...
    editBookmark(bookmarkTitle, bookmarkTags);
...
{#if showModal}
  ...
  >
    ...
    >
      <h2 class="text-2xl mb-2">Edit Bookmark</h2>
      ...
      <div class="flex justify-end gap-3 items-center mt-5">
        ...
        <Button onclick={onAddButtonClicked} icon="ic:outline-edit">Edit</Button
        >
      </div>
    </div>
  </div>
